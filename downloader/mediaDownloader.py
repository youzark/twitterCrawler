...
one with tag=* , contain multiple sub m3u8 represents different resolution
    ...
    with ThreadPoolExecutor(max_workers=threadNum) as ex:
        while True:
            if not workQueue.empty():
                ...
                if str(item.__class__) == "<class 'tweetVid.tweetVid'>":
                    ex.submit(_downloadM3u8,item.url,item.fileName,item.filePath)
            else:
...
#     logging.info("Pic Downloader Closed")
def main():
    print(_parseM3u8("https://video.twimg.com/ext_tw_video/1505580634068205573/pu/pl/ZvLOy2FtR8k4fxIy.m3u8?tag=12&container=fmp4"))
